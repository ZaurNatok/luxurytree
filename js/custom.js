...
to_top_fixed();
$(window).scroll(function(){to_top_fixed()});
*/
/*---header fixed end---*/
...
//  Smooth Scroll
    ...
    $(".nav-btn, .navigation__btn").on("click", function(){
...
$(document).ready(function(){
/*---error phone---*/
function require_phone(input_phone){
    if (input_phone.val() == +7 || input_phone.val().toString().indexOf('_') != -1 || input_phone.val().toString().indexOf('.') != -1 || input_phone.val() == ''){
        input_phone.parent().addClass('error');
    }
    else{
        input_phone.parent().removeClass('error');
    };
};
$('[data-require="require_phone"]').focusout(function(){
    require_phone($(this));
});
/*---error phone end---*/
/*---error mail---*/
function require_mail(input_mail){
    if(input_mail.val() != ''){
	var pattern_mail = /^[\w-\.]+@[\w-]+\.[a-z]{2,3}$/i;
        if(pattern_mail.test(input_mail.val())){
            input_mail.parent().removeClass('error');
        }
        else{
            input_mail.parent().addClass('error');
        };
    }
    else{
        input_mail.parent().addClass('error');
...
$('[data-require="require_mail"]').focusout(function(){
    require_mail($(this));
});
/*---error mail end---*/
/*---error text---*/
function require_text(input_text){
    if(input_text.val() != ''){
        input_text.parent().removeClass('error');
    }
    else{
        input_text.parent().addClass('error');
    };
};
$('[data-require="require_text"]').focusout(function(){
    require_text($(this));
});
/*---error text end---*/
/*---empty input---*/
function empty_input(empty_form){
...
$('input').focusout(function(){
    empty_input($(this).parents('form'));
});
/*---empty input end
/*---send form---*/
function send_form(send_form_form){
    targetParam = send_form_form.attr('data-send_target');
    send_form_form.find('[data-require="require_phone"]').each(function(){
	require_phone($(this));
    });
    send_form_form.find('[data-require="require_mail"]').each(function(){
	require_mail($(this));
    });
    send_form_form.find('[data-require="require_text"]').each(function(){
	require_text($(this));
    });
    if($(send_form_form.find('.error')).length){
	/*error true*/
    }
    else{
	/*error false*/
	send_form_form.addClass('submitted');
	AjaxFormSend('',''+send_form_form.attr('id')+'','/lux_order.php');
    }
}
$('[data-js="send_form"]').on('click', function(){
    send_form($(this).parents('form'));
});
...
$('.lightgallery1').lightGallery({
    selector: '.lightgallery1__item',
    thumbnail:true
});

$('.lightgallery2').lightGallery({
    selector: '.lightgallery2__item',
    thumbnail:true
});
/*
$(".box3__a_link").on("click", function(e){
    e.preventDefault();
    $(this).parents('.box3').find('.box3__link').click();
});
*/
